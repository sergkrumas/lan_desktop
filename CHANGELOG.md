...
- добавлен чат из примеров Qt, который я портировал на PyQt. В процессе пришлось отказаться от QCBORStreamWriter и QCBORStreamReader и воспользоваться модулем cbor2, так как питоновская обёртка QCBORStreamWriter не видит некоторых Qt-шных методов объекта, а питоновская обёртка QCBORStreamReader не даёт прочитать записанную строку через PyQt впринципе, выдавая ошибку питоновской обёртки - SystemError: sipBuildResult(): invalid format string "NF". Модуль cbor2 работает так же просто как и модуль json.
...
### 3й раунд разработки, версия 0.91 (30 марта 2024 - 1 апреля 2025)
...
### 4й раунд разработки, окончательная полировка, версия 0.92 (1 апреля 2025 - ??????????????)
- (17 май 25) зависание: копия программы на Windows зависает, когда я включаю копию программы на стороне Linux
    - причина, видимо, в этом: при приветствии пиров происходит попытка записать список пиров `Globals.update_peers_list` и для этого linux-копия начинала создавать дополнительные файлы-локи, видимо потому, что записывается этот список из потока чтения сокета, а этих потоков может быть много
